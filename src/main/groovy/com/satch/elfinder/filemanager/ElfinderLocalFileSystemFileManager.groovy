package com.satch.elfinder.filemanager
...
class ElfinderLocalFileSystemFileManager implements ElFinderFileManager {
	...
	String getRoot(){
		//TODO need display folder by current user restriction
		return fileManagerService.workareaFile.path//workFolderForCurrentUser
	}
...
//	Map uploadFile(CommonsMultipartFile multiPartFile, String targetDir) {
	Map uploadFile(def multiPartFile, String targetDir) {
	...
	String getMimeTypeForFile(File file) {
		...
		return com.satch.elfinder.MimeTypeMappings.forExtension(file.name?.split(".")?.last())
	}
}
